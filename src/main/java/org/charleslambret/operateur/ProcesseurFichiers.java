...
public class ProcesseurFichiers {
    ...
    private void processFile(Path filePath) {
        ...
        try (BufferedReader reader = Files.newBufferedReader(filePath);
            ...
            reader.lines().forEach(line -> new ProcesseurLignes().processLine(line, writer));
        } catch (IOException e) {
            System.out.println("Erreur lors du traitement du fichier : " + filePath.getFileName());
        }
    }

    public void processOperations(List<OperationData> operations, String directory) throws IOException {
        for (OperationData opData : operations) {
            if ("OP".equals(opData.getType())) {
                ...
                try (BufferedWriter writer = Files.newBufferedWriter(outputPath, StandardOpenOption.CREATE)) {
                    ...
                    try {
                        OperationStrategy strategy = new OperationFactory().getOperation(opData.getOperator());
